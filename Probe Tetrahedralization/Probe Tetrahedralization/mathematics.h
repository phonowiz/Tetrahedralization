...
struct segment3d {
    ...
    segment3d() = delete;
    segment3d(uint32_t p1_, uint32_t p2_):id(current_id++), p1(p1_), p2(p2_)
    {
        std::array<uint32_t, 2> k = {p1, p2};
    ...
    {
        return std::tie(p1, p2) == std::tie(rhs.p1, rhs.p2);
    }
};
...
namespace std
{
    ...
    template<>
    struct std::hash<segment3d>
    {
        std::size_t operator()(const segment3d& s) const noexcept
        {
            std::array<uint32_t, 2> k = {s.p1, s.p2};
            std::sort(k.begin(), k.end(), std::greater<uint32_t>());
            unsigned char * b = reinterpret_cast<unsigned char*>(&k);
            std::size_t h = 0;
            for(int i = 0; i < sizeof(k); ++i)
                                ...
                                double b1)
{
    ...
    if (det < -1e-4)
    {
        throw std::logic_error("in exact arithmetics, 'det' cannot be negative; but we must take into account floating-point errors");
    }
    if (det < 0.0)
    {
        det = 0.0;
...
using TriangleIntersectsFloats = TriangleIntersects<std::vector<float>>;
}  // namespace moeller
}  // namespace threeyd
